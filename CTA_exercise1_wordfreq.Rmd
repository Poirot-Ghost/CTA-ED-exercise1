...
# show me
# race-related words
race_words <- c("race", "racial", "ethnicity", "ethnic", "diversity", "discrimination", 
                "oppression", "privilege", "systemic racism", "bias", "colorism", 
                "microaggression", "prejudice", "stereotype", "inclusion", "exclusion",
                "xenophobia", "multicultural", "intersectionality", "marginalized")

race_ethnic_words <- c(
  # black
  "black", "African", "African American", "Afro", "Afro-American", "BIPOC", 
  "Black Lives Matter", "BLM", "slavery", "segregation", "Jim Crow", 
  "blackface", "civil rights", "Tuskegee", "reparations",

  # white
  "white", "Caucasian", "Euro-American", "whiteness", "white privilege", 
  "white supremacy", "colonialism", "settler", "imperialism", 

  # Asian
  "Asian", "Asian American", "Chinese", "Japanese", "Korean", "Filipino", "Vietnamese",
  "South Asian", "Indian", "Pakistani", "Bangladeshi", "Desi", "Pacific Islander",
  "East Asian", "Southeast Asian", "xenophobia", "yellowface", "model minority",
  "anti-Asian", "Stop Asian Hate",

  # Latin
  "Latino", "Latina", "Latinx", "Hispanic", "Chicano", "Chicana", "Chicanx", 
  "Mexican", "Puerto Rican", "Cuban", "Dominican", "Salvadoran", "Central American",
  "South American", "mestizo", "immigration", "border", "ICE", "deportation", 
  "DACA", "Dreamer", "undocumented",

  # Original
  "Indigenous", "Native American", "First Nations", "Aboriginal", "tribe", "reservation",
  "Indian", "Navajo", "Cherokee", "Lakota", "Sioux", "Apache", "Inuit", "Métis",
  "colonization", "genocide", "land rights", "sovereignty", "stolen land", 

  # Mid East
  "Middle Eastern", "Arab", "Arab American", "Persian", "Iranian", "Turk", "Syrian",
  "Lebanese", "Palestinian", "Muslim", "Islamophobia", "Hijab", "Refugee", "diaspora"
)

racist_terms <- c("racial slur", "racist", "bigotry", "hate speech", "neo-nazi", 
                  "KKK", "klan", "white nationalist", "hate crime", "lynching", 
                  "apartheid", "ghetto", "segregation", "redlining", "Jim Crow", 
                  "racial profiling", "stop and frisk", "police brutality", 
                  "hate group", "alt-right", "dog whistle")

race_related_words <- c(race_words, race_ethnic_words, racist_terms)

edbf_term_counts$raceword <- as.integer(grepl(paste(race_related_words, collapse="|"), 
                                            x = edbf_term_counts$word))
...
```{r}
#get counts by year and word
edbf_counts_race <- edbf_term_counts %>%
  group_by(year) %>%
  mutate(year_total = sum(n)) %>%
  filter(raceword==1) %>%
...
ggplot(edbf_counts_race, aes(year, sum_race / year_total, group=1)) +
  ...
  geom_text(aes(x=2020.1, label="Murder of George Floyd", y=.0025), 
            colour="black", angle=90, text=element_text(size=8)) +
  xlab("Year") +
  ylab("% race-related words") +
  scale_y_continuous(labels = scales::percent_format(),
