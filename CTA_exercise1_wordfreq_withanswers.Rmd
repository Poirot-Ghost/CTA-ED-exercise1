...
1.1 First, just for practice, create a new dataset called 'youngestedbf'. Select variables 'year', 'genre' and 'age category', and filter the books so your new dataset only includes the 2 youngest age category (targeted towards 4-7 years old and 6-10 years old).
```{r}
names(edbfdata) # check again the variable names

youngestedbf <- edbfdata %>% # specify the original dataset you are working off
  select(year, genre, age_category) %>% # select the columns of interests
  filter(age_category == "AGE 4 - 7" | age_category == "AGE 6 - 10") # select the rows of interest

# let's check
library(questionr)
freq(youngestedbf$age_category)

# to check your code is working as you develop it, it can be useful to test it regularly, for instance by executing %>% head() after each new line.
```
